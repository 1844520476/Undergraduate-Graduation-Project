...
if __name__ == "__main__":
    ...
    #   数据集存放的路径
    # ----------------------------------------------------#
    # TODO 1.（训练阶段）选择微调数据集地址
    dataset_path = "datasets/Mac10"
    # ----------------------------------------------------#
    #   输入图像的大小，默认为105,105,3
    # ----------------------------------------------------#
    # TODO 2.将决定输入图像大小与微调后网络权重输入端口大小（使用权重时要注意保证大小一致）
    input_shape = [512, 512, 3] # 高 宽 RGB三通道
    print(f'(train)input_shape:{input_shape}')
    # ----------------------------------------------------#
    ...
    # TODO 3.权重输出地址
    logs_path = f'logs/mac_50_10_1'
    # -------------------------------#
    #   用于指定是否使用VGG预训练权重：vgg骨干网络预训练权重加载在nets/vgg.py中
    #   有两种获取方式
    #   1、利用百度网盘下载后放入
    #      ./logs/
    #   2、直接运行自动下载
    ...
    if True:
        ...
        for epoch in range(Init_epoch, Freeze_epoch):
            fit_one_epoch(logs_path, model_train, model, loss, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val,
                          Freeze_epoch, Cuda)
            lr_scheduler.step()
