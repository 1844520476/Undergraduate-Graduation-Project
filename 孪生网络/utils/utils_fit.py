...
def fit_one_epoch(logs_path, model_train, model, loss, optimizer, epoch, epoch_step, epoch_step_val, gen, genval, Epoch, cuda):
    total_loss = 0
    total_accuracy = 0
    ...
    torch.save(model.state_dict(), f'{logs_path}/ep%03d-loss%.3f-val_loss%.3f.pth' % (
        (epoch + 1 + 14), total_loss / epoch_step, val_loss / epoch_step_val))
