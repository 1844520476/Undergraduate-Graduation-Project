...
def main():
    test_num = 1
    # while Ture:
    # INPUT()函数必须放在上面，不然会报错：TypeError: 'Tensor' object is not callable
    cls = int(input(rf'请输入检测类别数：'))
    Input = torch.randn(1, 3, 224, 224)
    net = int(input(rf'chose net:[1:50,2:101,3:152]:'))
    while True:
        if net == 1:
            resnet = ResNet50(cls)
            break
        elif net == 2:
            resnet = ResNet101(cls)
            break
        elif net == 3:
            resnet = ResNet152(cls)
            break
        else:
            net = int(input(rf'chose net:[1:50,2:101,3:152]:'))
    out = resnet(Input)
    print(f'1. net:{resnet}\n1.2 test[{test_num}]cls:{cls}')
    print(f'2.1 out:{out}\n2.2 shape of out:{out.shape}')
    # test_num += 1
    return resnet
